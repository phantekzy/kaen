...
import { Loader2, X, Send, MessageCircle, Trash2, Edit3, ChevronDown, ChevronUp, Lock, ArrowLeft, Plus, PlusSquare, ThumbsUp, ThumbsDown } from "lucide-react";
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {comment.children?.length > 0 && (
                            <button onClick={() => setShowReplies(!showReplies)} className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-pink-600">
                                ...
                                {comment.children.length} {comment.children.length === 1 ? 'replie' : 'replie'}
                            </button>
                        )}
                    </div>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className="mt-4 border-l border-white/5 ml-2 pl-4 space-y-6">
                                {comment.children.map((child: any) => (
                                    <RecursiveComment key={child.id} comment={child} currentUserId={currentUserId} onReply={onReply} onRefresh={onRefresh} onError={onError} />
                                ))}
                            </div>
                        </motion.div>
                    )}
        ...
        <div className={`w-full pb-32 ${selectedPostId ? "lg:h-screen lg:overflow-hidden" : ""}`}>
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                {posts && posts.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-40 border border-dashed border-white/10 rounded-[40px] bg-zinc-900/5 text-center">
                        <PlusSquare size={32} className="text-zinc-700 mb-8" />
                        <h3 className="text-white font-black uppercase tracking-[0.3em] text-lg mb-4">Sector Empty</h3>
                        <button onClick={() => navigate('/create-post')} className="px-10 py-4 bg-white text-black text-[10px] font-black uppercase rounded-2xl">Create First Post</button>
                    </div>
                )}
            </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex flex-col lg:flex-row bg-black/95 backdrop-blur-xl">
                        <div className="hidden lg:flex flex-1 relative flex-col overflow-y-auto p-12">
                            ...
                            <div className="max-w-4xl mx-auto w-full space-y-10">
                                {activePost.image_url && <img src={activePost.image_url} className="w-full h-auto rounded-3xl shadow-2xl" alt="" />}
                        ...
                        <motion.div
                            ...
                            className="w-full lg:w-[480px] bg-zinc-950 border-t lg:border-t-0 lg:border-l border-white/10 flex flex-col h-full shadow-2xl ml-auto"
                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-8 custom-scrollbar">
                                {isLoadingComments ? <Loader2 className="animate-spin text-pink-600 mx-auto" /> :
                                    commentTree.map(comment => (
                                        <RecursiveComment key={comment.id} comment={comment} currentUserId={user?.id} onReply={(c: any) => setReplyingTo(c)} onRefresh={refetchComments} onError={(msg: string) => showToast(msg, 'error')} />
                                    ))}
                            </div>
